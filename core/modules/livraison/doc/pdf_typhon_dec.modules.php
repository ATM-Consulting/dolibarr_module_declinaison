 ...
 */
class pdf_typhon_dec extends ModelePDFDeliveryOrder
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "typhon_dec";
		$this->description = $langs->trans("DocumentModelTyphonDec");
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				*/
				...
				$nexY = $tab_top + 11;
/*
				$conf->global->DECLINAISON_COMPACT_LINES_BREAK_ON_TITLE = 0;
				$parameters=array();
				$a=new ActionsDeclinaison($object->db);
				$a->dec_replace_line($parameters, $object->commande, $action, $hookmanager);
				var_dump($object->commande);exit;*/
				// Loop on each lines
				for ($i = 0 ; $i < $nblines ; $i++)
				{
